...
-- Reviews (student/member testimonials, e.g. after belt exams)
...
BEGIN
    ...
    (N'Пятько Павел Станиславович', N'6 Дан WMAO, 5 Дан JKS,  4 Дан WKF | Главный тренер клуба',
     N'/img/piatsko.svg',
     N'Высшее образование: БГУФК (тренер по каратэ), БНТУ (инженер). 6 Дан WMAO; 5 Дан JKS; 4 Дан WKF; 3 Дан JKF; 2 Дан JKA, WSKF. Национальный судья. Председатель Ассоциации "Минская Федерация Каратэ".',
     1);

    INSERT INTO instructors (name, title, photo, description, sortOrder) VALUES
    (N'Кость Михаил Богданович', N'3 Дан MKF, 2 Дан JKS, 2 Дан WKF, WMAO',
     N'/img/kosts.svg',
     N'Высшее образование: БГУФК (тренер по каратэ). 3 Дан MKF; 2 Дан JKS; 2 Дан WKF, WMAO; 1 Дан JKF. Мастер спорта международного класса РБ',
     2);

    INSERT INTO instructors (name, title, photo, description, sortOrder) VALUES
    (N'Винцукевич Дарья Петровна', N'3 Дан MKF, 2 Дан JKS, 1 Дан ISKF | КМС РБ',
     N'/img/vintsukevich.svg',
     N'Высшее образование: БГУФК (тренер по каратэ), БГУ (химик-эколог). 3 Дан MKF; 2 Дан JKS; 1 Дан ISKF. Кандидат в мастера спорта РБ.',
     3);
END
GO

-- Reviews (from sanker.by, "Экзамен от первого лица")
IF NOT EXISTS (SELECT 1 FROM reviews)
BEGIN
    INSERT INTO reviews (author, text, photo, sortOrder) VALUES
    (N'Артём',
     N'Для меня стало неожиданностью, что мой организм так стрессанёт…
...
Искренне…',
     N'/uploads/reviews/review-artem-seed.png', 1);

    INSERT INTO reviews (author, text, photo, sortOrder) VALUES
    (N'Александр',
     N'Когда происходит такого рода проверка знаний, это можно принимать за испытание, ты понимаешь, что живёшь здесь и сейчас и все твои действия, эмоции, реакции на окружение необратимы.
...
Спасибо большое всем наставникам за организацию, я впечатлён и рад что с вами!!!',
     N'/img/default-instructor.svg', 2);

    INSERT INTO reviews (author, text, photo, sortOrder) VALUES
    (N'Алексей',
     N'Первый экзамен на 9 КЮ я сдавал ещё в 1996 году у Ермакова В.Н.
...
Огромное спасибо Клубу за этот праздник, тренерскому составу – за организацию и нашу подготовку, и всем бойцам, с которыми мы вместе идём по пути познания!',
     N'/img/default-instructor.svg', 3);

    INSERT INTO reviews (author, text, photo, sortOrder) VALUES
    (N'Павел',
     N'Я думал, что я не волновался, но после душа меня отрубило как после недельной дозы коньяка!
Спасибо за позитивный стресс!',
     N'/img/default-instructor.svg', 4);

    INSERT INTO reviews (author, text, photo, sortOrder) VALUES
    (N'Александр',
     N'Экзаменов по жизни было немало; в основном всё теория, и только один водительская практика.
...
В общем, теперь достоин белого пояса.',
     N'/img/default-instructor.svg', 5);
END
GO
